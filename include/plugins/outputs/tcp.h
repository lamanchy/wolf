 ...
 private:
  void do_write() {
    while (true) {
      ...
      try {
        ...
        logger.warn << "sending of tcp message failed: " << e.what() << std::endl;
        do_connect();
        continue;
      }
  ...
  void do_connect() {
    try {
      ...
      logger.info << "Successfully connected to " << host << ":" << port << std::endl;
    } catch (std::exception &e) {
      logger.warn << "tcp connect failed: " << e.what() << std::endl;
